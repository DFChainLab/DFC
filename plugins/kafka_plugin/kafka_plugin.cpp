/**
 *  @file
 *  @copyright defined in dfc/LICENSE.txt
 */
//
#include <stdlib.h>
#include <dfcio/kafka_plugin/kafka_producer.hpp>
#include <dfcio/kafka_plugin/kafka_plugin.hpp>
#include <dfcio/kafka_plugin/kafka_types.hpp>
...
namespace dfcio {
    ...
    using dfcio::kafka_block_accept_st;
...
////////////
    ...
    void kafka_plugin::set_program_options(options_description &cli, options_description &cfg) {
        cfg.add_options()
                ("kafka-trx_accept_topic", bpo::value<std::string>()->default_value("dfcio.trx-accept"),
                 "The topic for accepted transaction.")
                ("kafka-trx_applied_topic", bpo::value<std::string>()->default_value("dfcio.trx-applied"),
                 "The topic for appiled transaction.")
                ("kafka-trx_transfer_topic", bpo::value<std::string>()->default_value("dfcio.trx-transfer"),
                 "The topic for transfer transaction.")
                ("kafka-block_accepted_topic", bpo::value<std::string>()->default_value("dfcio.block-accept"),
                ...
                ("kafka-queue-size", bpo::value<uint32_t>()->default_value(256),
                 "The target queue size between noddfc and kafka plugin thread.")
    ...
    void kafka_plugin::plugin_initialize(const variables_map &options) {
        ...
        try {
            ...
            if (options.count("kafka-uri")) {
                ...
                wlog( "dfcio::kafka_plugin configured, but no --kafka-uri specified." );
