...
#include <dfcio/bnet_plugin/bnet_plugin.hpp>
...
using dfcio::public_key_type;
using dfcio::chain_id_type;
using dfcio::block_id_type;
using dfcio::block_timestamp_type;
using std::string;
using dfcio::sha256;
using dfcio::signed_block_ptr;
using dfcio::packed_transaction_ptr;
using std::vector;
...
namespace dfcio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              DFC_THROW(block_validate_exception, "bad block" );
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           DFC_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         DFC_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
   ...
   void session::on( const packed_transaction_ptr& p ) {
      ...
      if (!p) {
        ...
        DFC_THROW(transaction_exception, "bad transaction");
      }
