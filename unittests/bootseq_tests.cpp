...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(dfcio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create dfcio.msig and dfcio.token
        create_accounts({N(dfcio.msig), N(dfcio.token), N(dfcio.ram), N(dfcio.ramfee), N(dfcio.stake), N(dfcio.vpay), N(dfcio.bpay), N(dfcio.saving) });

        // Set code for the following accounts:
        //  - dfcio (code: dfcio.bios) (already set by tester constructor)
        //  - dfcio.msig (code: dfcio.msig)
        //  - dfcio.token (code: dfcio.token)
        set_code_abi(N(dfcio.msig), dfcio_msig_wast, dfcio_msig_abi);//, &dfcio_active_pk);
        set_code_abi(N(dfcio.token), dfcio_token_wast, dfcio_token_abi); //, &dfcio_active_pk);

        // Set privileged for dfcio.msig and dfcio.token
        set_privileged(N(dfcio.msig));
        set_privileged(N(dfcio.token));

        // Verify dfcio.msig and dfcio.token is privileged
        const auto& dfcio_msig_acc = get<account_object, by_name>(N(dfcio.msig));
        BOOST_TEST(dfcio_msig_acc.privileged == true);
        const auto& dfcio_token_acc = get<account_object, by_name>(N(dfcio.token));
        BOOST_TEST(dfcio_token_acc.privileged == true);


        // Create SYS tokens in dfcio.token, set its manager as dfcio
        ...
        create_currency(N(dfcio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to dfcio.system
        issue(N(dfcio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set dfcio.system to dfcio
        set_code_abi(config::system_account_name, dfcio_system_wast, dfcio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(dfcio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "dfcio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), dfcio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), dfcio_assert_message_exception);
