...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(dfcio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(dfcio.token), dfcio_token_wast );
   c.set_abi( N(dfcio.token), dfcio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(dfcio.token), N(create), N(dfcio.token), mutable_variant_object()
              ("issuer",       "dfcio" )
   ...
   cr = c.push_action( N(dfcio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
