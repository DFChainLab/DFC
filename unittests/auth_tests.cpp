...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for dfcio reqauth action with alice's spending key
   chain.link_authority("alice", "dfcio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "dfcio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with dfcio reqauth
   chain.unlink_authority("alice", "dfcio", "reqauth");
   ...
   // Link authority for any dfcio action with alice's scud key
   chain.link_authority("alice", "dfcio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "dfcio", "first",  "reqauth");
...
try {
   ...
   // Creating account with dfcio. prefix with privileged account
   chain.create_account("dfcio.test1");

   // Creating account with dfcio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("dfcio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'dfcio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "dfcio", "dfcio.any", "reqauth" );
   chain.link_authority( "bob", "dfcio", "dfcio.any", "reqauth" );

   /// this should succeed because dfcio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = dfcio::chain::resource_limits::resource_usage_object;
   using by_owner = dfcio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link dfcio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
