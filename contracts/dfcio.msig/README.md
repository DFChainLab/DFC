...
Prerequisites:
   - dfcio.token contract installed to dfcio.token account, dfcio.msig contract installed on dfcio.msig account which is a privileged account.
   - account 'treasury' is the issuer of DFC token.
...
````
$ cldfc multisig propose test '[{"actor": "treasury", "permission": "active"}]' '[{"actor": "treasury", "permission": "active"}]' dfcio.token issue '{"to": "tester", "quantity": "1000.0000 DFC", "memo": ""}' -p tester
...
````
$ cldfc multisig review tester test -p treasury
{
  "proposal_name": "test",
  "requested_approvals": [{
      "actor": "treasury",
...
````
$ cldfc multisig approve tester test '{"actor": "treasury", "permission": "active"}' -p treasury
...
````
$ cldfc multisig exec tester test -p tester
