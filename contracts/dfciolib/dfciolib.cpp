...
namespace dfcio {
   ...
   void set_blockchain_parameters(const dfcio::blockchain_parameters& params) {
      char buf[sizeof(dfcio::blockchain_parameters)];
      dfcio::datastream<char *> ds( buf, sizeof(buf) );
      ds << params;
      set_blockchain_parameters_packed( buf, ds.tellp() );
   }

   void get_blockchain_parameters(dfcio::blockchain_parameters& params) {
      char buf[sizeof(dfcio::blockchain_parameters)];
      size_t size = get_blockchain_parameters_packed( buf, sizeof(buf) );
      dfcio_assert( size <= sizeof(buf), "buffer is too small" );
      dfcio::datastream<const char*> ds( buf, size_t(size) );
      ds >> params;
   }
   ...
   private:
      ...
      public:
         ...
         {
            dfcio_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return dfcio::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = dfcio::memory_heap.malloc(count*size);
...
{
   return dfcio::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return dfcio::memory_heap.free(ptr);
}
