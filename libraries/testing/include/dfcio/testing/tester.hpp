...
#define REQUIRE_MATCHING_OBJECT(left, right) { auto a = fc::variant( left ); auto b = fc::variant( right ); BOOST_REQUIRE_EQUAL( true, a.is_object() ); \
   BOOST_REQUIRE_EQUAL( true, b.is_object() ); \
   auto filtered = ::dfcio::testing::filter_fields( a.get_object(), b.get_object() ); \
...
dfcio::chain::asset core_from_string(const std::string& s);
...
namespace dfcio { namespace testing {
  ...
  /**
   * Utility predicate to check whether an dfcio_assert message is equivalent to a given string
   */
  struct dfcio_assert_message_is {
     dfcio_assert_message_is( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }

     bool operator()( const dfcio_assert_message_exception& ex );

     string expected;
  };

  /**
   * Utility predicate to check whether an dfcio_assert message starts with a given string
   */
  struct dfcio_assert_message_starts_with {
     dfcio_assert_message_starts_with( const string& msg )
           : expected( "assertion failure with message: " ) {
        expected.append( msg );
     }
  ...
  /**
   * Utility predicate to check whether an dfcio_assert_code error code is equivalent to a given number
   */
  struct dfcio_assert_code_is {
     dfcio_assert_code_is( uint64_t error_code )
           : expected( "assertion failure with error code: " ) {
        expected.append( std::to_string(error_code) );
     }

     bool operator()( const dfcio_assert_code_exception& ex );
