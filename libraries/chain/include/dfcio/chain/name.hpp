...
#define N(X) dfcio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<dfcio::chain::name> : private hash<uint64_t> {
      typedef dfcio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const dfcio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, dfcio::chain::name& check);
} // fc


FC_REFLECT( dfcio::chain::name, (value) )
