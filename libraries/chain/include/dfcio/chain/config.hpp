...
namespace dfcio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(dfcio);
const static uint64_t null_account_name      = N(dfcio.null);
const static uint64_t producers_account_name = N(dfcio.prods);
...
const static uint64_t dfcio_auth_scope       = N(dfcio.auth);
const static uint64_t dfcio_all_scope        = N(dfcio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t dfcio_any_name = N(dfcio.any);
const static uint64_t dfcio_code_name = N(dfcio.code);
...
const static dfcio::chain::wasm_interface::vm_type default_wasm_runtime = dfcio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace dfcio::chain::config

constexpr uint64_t DFC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / dfcio::chain::config::percent_100;
}

template<typename Number>
Number DFC_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + dfcio::chain::config::percent_100 - dfcio::chain::config::percent_1)  / dfcio::chain::config::percent_100;
}
