...
class Utils:
    ...
    FscClientPath="programs/cldfc/cldfc"
    MiscFscClientArgs="--no-auto-kdfcd"

    FscWalletName="kdfcd"
    FscWalletPath="programs/kdfcd/"+ FscWalletName

    FscServerName="noddfc"
    FscServerPath="programs/noddfc/"+ FscServerName

    FscLauncherPath="programs/dfcio-launcher/dfcio-launcher"
    MongoPath="mongo"
    ShuttingDown=False
    CheckOutputDeque=deque(maxlen=10)

    FscBlockLogPath="programs/dfcio-blocklog/dfcio-blocklog"
    ...
    def getBlockLog(blockLogLocation, silentErrors=False, exitOnError=False):
        ...
        cmd="%s --blocks-dir %s --as-json-array" % (Utils.FscBlockLogPath, blockLogLocation)
