...
###############################################################
# noddfc_run_remote_test
#  Tests remote capability of the noddfc_run_test. Test will setup cluster and pass nodes info to noddfc_run_test. E.g.
#  noddfc_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/noddfc_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("noddfc", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killFscInstances, False, False, killAll, dumpErrorDetails)
