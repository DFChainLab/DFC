...
# --kill-count <noddfc instances to kill>
...
try:
    ...
    Print("Stand up DFC wallet kdfcd")
    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.dfcioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    dfcioAccount=cluster.dfcioAccount
    ...
    if not cluster.createAccounts(dfcioAccount):
    ...
    if cluster.killSomeFscInstances(killCount, killSignal) is False:
        errorExit("Failed to kill Fsc instances")
    Print("noddfc instances killed.")
    ...
    if cluster.relaunchFscInstances() is False:
        errorExit("Failed to relaunch Fsc instances")
    Print("noddfc instances relaunched.")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killFscInstances, killFscInstances, keepLogs, killAll, dumpErrorDetails)
