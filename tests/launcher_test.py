...
try:
    ...
    if not dontLaunch:
        ...
        killFscInstances=False

        print("Stand up walletd")
        if walletMgr.launch() is False:
            cmdError("%s" % (WalletdName))
            errorExit("Failed to stand up dfc walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.dfcioAccount,cluster.defproduceraAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.dfcioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.dfcioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.dfcioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.dfcioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.dfcioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.dfcioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountFscBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountFscBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
