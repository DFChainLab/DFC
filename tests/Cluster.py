...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="dfcio-ignition-wd/bootlog.txt"
    __configDir="etc/dfcio/"
    __dataDir="var/lib/"
    __fileDivider="================================================================="

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kdfcd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="DFCtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kdfcd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: dfc server host
        port: dfc server port
        walletHost: dfc wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures dfc mongo plugin
        ...
        self.dfcioAccount=self.defProducerAccounts["dfcio"]= Account("dfcio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNoddfcArgs=None, useBiosBootFile=True, specificExtraNoddfcArgs=None):
        """Launch cluster.
        ...
        extraNoddfcArgs: string of arguments to pass through to each nodoes instance (via --noddfc flag on launcher)
        ...
        specificExtraNoddfcArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-noddfc flags on launcher), example: { "5" : "--plugin dfcio::test_control_api_plugin" }
        """
        assert(isinstance(topo, str))

        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.FscServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on noddfc default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.FscLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        noddfcArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            noddfcArgs += " --plugin dfcio::wallet_api_plugin"
        if self.enableMongo:
            noddfcArgs += " --plugin dfcio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNoddfcArgs is not None:
            assert(isinstance(extraNoddfcArgs, str))
            noddfcArgs += extraNoddfcArgs
        if Utils.Debug:
            noddfcArgs += " --contracts-console"

        if noddfcArgs:
            cmdArr.append("--noddfc")
            cmdArr.append(noddfcArgs)

        if specificExtraNoddfcArgs is not None:
            assert(isinstance(specificExtraNoddfcArgs, dict))
            for nodeNum,arg in specificExtraNoddfcArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-noddfc")
                cmdArr.append(arg)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.FscServerName, totalNodes, len(nodes)))
            return False
        ...
        self.dfcioAccount=self.defProducerAccounts["dfcio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.FscClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.FscClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on dfc server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextFscIdx: %d, n: %d" % (nextFscIdx, n))
                nextFscIdx=(nextFscIdx + 1)%count
                if not self.nodes[nextFscIdx].killed:
                    #Utils.Print("nextFscIdx: %d" % (nextFscIdx))
            ...
            #Utils.Print("nextFscIdx: %d, count: %d" % (nextFscIdx, count))
            node=self.nodes[nextFscIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.FscServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on dfc node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getFscBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.dfcioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse dfcio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by dfcio-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down dfc cluster.")
            return None
        ...
        dfcioName="dfcio"
        dfcioKeys=producerKeys[dfcioName]
        dfcioAccount=Account(dfcioName)
        dfcioAccount.ownerPrivateKey=dfcioKeys["private"]
        dfcioAccount.ownerPublicKey=dfcioKeys["public"]
        dfcioAccount.activePrivateKey=dfcioKeys["private"]
        dfcioAccount.activePublicKey=dfcioKeys["public"]
        producerKeys.pop(dfcioName)

        if not walletMgr.importKey(dfcioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (dfcioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="dfcio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"dfcio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init dfcio transfer")
            opts="--permission dfcio@active"
            if name != "dfcio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from dfcio.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        ...
        trans=biosNode.publishContract(dfcioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, dfcioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission dfcio@active"
                    myTrans=biosNode.pushMessage("dfcio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("dfcio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but dfcio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "dfcio"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        dfcioTokenAccount=copy.deepcopy(dfcioAccount)
        dfcioTokenAccount.name="dfcio.token"
        trans=biosNode.createAccount(dfcioTokenAccount, dfcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dfcioTokenAccount.name))
            return None

        dfcioRamAccount=copy.deepcopy(dfcioAccount)
        dfcioRamAccount.name="dfcio.ram"
        trans=biosNode.createAccount(dfcioRamAccount, dfcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dfcioRamAccount.name))
            return None

        dfcioRamfeeAccount=copy.deepcopy(dfcioAccount)
        dfcioRamfeeAccount.name="dfcio.ramfee"
        trans=biosNode.createAccount(dfcioRamfeeAccount, dfcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dfcioRamfeeAccount.name))
            return None

        dfcioStakeAccount=copy.deepcopy(dfcioAccount)
        dfcioStakeAccount.name="dfcio.stake"
        trans=biosNode.createAccount(dfcioStakeAccount, dfcioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (dfcioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(dfcioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=dfcioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (dfcioTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to dfcio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (dfcioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to dfcio contract.")
            return None
        ...
        Utils.Print("Verify dfcio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountFscBalanceStr(dfcioAccount.name)
        if expectedAmount != actualAmount:
            Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                        (expectedAmount, actualAmount))
            return None

        contract="dfcio.system"
        contractDir="contracts/%s" % (contract)
        wasmFile="%s.wasm" % (contract)
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (dfcioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (dfcioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (dfcioTokenAccount.name, name))
                return None
    ...
    @staticmethod
    def pgrepFscServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.FscServerName)
    ...
    @staticmethod
    def pgrepFscServerPattern(nodeInstance):
        dataLocation=Cluster.__dataDir + Cluster.nodeExtensionToName(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of FscInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepFscServers(timeout)
        if psOut is None:
        ...
        for i in range(0, totalNodes):
            pattern=Cluster.pgrepFscServerPattern(i)
            m=re.search(pattern, psOut, re.MULTILINE)
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.FscServerName, pattern))
                break
    ...
    def discoverBiosNodePid(self, timeout=None):
        psOut=Cluster.pgrepFscServers(timeout=timeout)
        pattern=Cluster.pgrepFscServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.FscServerName, pattern))
    ...
    # Kills a percentange of Fsc instances starting from the tail and update fscInstanceInfos state
    def killSomeFscInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.FscServerName, killSignal))
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster noddfc instances. allInstances will kill all noddfc instances running on the system."""
        cmd="%s -k 9" % (Utils.FscLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down dfc cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the dfc server
            cmd="pkill -9 %s" % (Utils.FscServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down dfc cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces noddfc instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down noddfc instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
