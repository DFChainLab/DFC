...
killAll()
{
  programs/dfcio-launcher/dfcio-launcher -k 15
}

cleanup()
{
    rm -rf etc/dfcio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/cldfc/cldfc get info)"
  verifyErrorCode "cldfc get info"
...
# stand up noddfc cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --noddfc \"--plugin dfcio::wallet_api_plugin\" $launcherOpts
programs/dfcio-launcher/dfcio-launcher --noddfc "--plugin dfcio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/cldfc/cldfc --port $port2 get block 1)"
    verifyErrorCode "cldfc get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/cldfc/cldfc create key)"
verifyErrorCode "cldfc create key"
...
PASSWORD_INITA="$(programs/cldfc/cldfc wallet create --name inita)"
verifyErrorCode "cldfc wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/cldfc/cldfc wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "cldfc wallet import"
programs/cldfc/cldfc wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "cldfc wallet import"
programs/cldfc/cldfc wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "cldfc wallet import"
programs/cldfc/cldfc wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "cldfc wallet import"
...
ACCOUNT_INFO="$(programs/cldfc/cldfc create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "cldfc create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/cldfc/cldfc get account testera)"
verifyErrorCode "cldfc get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/cldfc/cldfc transfer inita testera 975321 "test transfer")"
    verifyErrorCode "cldfc transfer"
   ...
   TRANS_INFO="$(programs/cldfc/cldfc --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "cldfc get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
