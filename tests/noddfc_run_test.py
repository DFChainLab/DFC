...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.dfcioAccount)
    ...
    contractDir="contracts/dfcio.token"
    wasmFile="dfcio.token.wasm"
    abiFile="dfcio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getFscAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get DFC account failed", raw=True)
    ...
    account=node.getFscAccount(defproduceraAccount.name, exitOnError=True)
