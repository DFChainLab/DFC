/**
 *  @file
 *  @copyright defined in dfcio/LICENSE.txt
 */
...
{
   try {
      app().set_version(dfcio::noddfc::config::version);

      auto root = fc::app_path();
      app().set_default_data_dir(root / "dfcio/noddfc/data" );
      app().set_default_config_dir(root / "dfcio/noddfc/config" );
      ...
      initialize_logging();
      ilog("noddfc version ${ver}", ("ver", app().version_string()));
      ilog("dfcio root is ${root}", ("root", root.string()));
      ilog("noddfc using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("noddfc data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();
